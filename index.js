...
app.post('/', encodeUrl, async (req, res) => {
    ...
    try {
        if (alreadyExist == 0) {
            await exec('node scripts/mint.js', {
                env: { USERWALLET: req.body.userAddress, EMAIL: req.body.email, ALCHEMY_MUMBAI_KEY:process.env.ALCHEMY_MUMBAI_KEY ,PRIVATE_KEY: process.env.PRIVATE_KEY, CONTRACT: process.env.CONTRACT }, function(error, stdout, stderr) {
                    if (error !== null) {
